...
DcOptions ConnectionCreator::get_default_dc_options(bool is_test) {
  ...
  // BeHappy: only use the configured MTProto port (10443). Telegram's
  // fallback ports (443, 80, 5222) are intentionally dropped to avoid
  // conflicts when MVSy and an HTTP server share the same host.
  //
  // The host is read from env BH_MVSY_HOST and falls back to the public
  // BeHappy hostname. Set BH_MVSY_HOST to point at a different deployment
  // (e.g. localhost during development, or a private cluster DNS name).
  // HostType::Url runs init_host_port which performs DNS resolution.
  const char *bh_mvsy_host_env = std::getenv("BH_MVSY_HOST");
  std::string bh_mvsy_host = (bh_mvsy_host_env && *bh_mvsy_host_env)
      ? std::string(bh_mvsy_host_env)
      : std::string("mvsy.behappy.rest");
  vector<int> ports = {10443};
#if TD_EMSCRIPTEN
  if (is_test) {
    add_ip_ports(1, {bh_mvsy_host}, {10443}, HostType::Url);
  } else {
    add_ip_ports(1, {bh_mvsy_host}, {10443}, HostType::Url);
  }
#else
  if (is_test) {
    add_ip_ports(1, {bh_mvsy_host}, ports, HostType::Url);
    add_ip_ports(2, {bh_mvsy_host}, ports, HostType::Url);
    add_ip_ports(3, {bh_mvsy_host}, ports, HostType::Url);

    // BeHappy: no IPv6 fallback
  } else {
    // BeHappy: all DCs point to our backend
    add_ip_ports(1, {bh_mvsy_host}, ports, HostType::Url);
    add_ip_ports(2, {bh_mvsy_host}, ports, HostType::Url);
    add_ip_ports(3, {bh_mvsy_host}, ports, HostType::Url);
    add_ip_ports(4, {bh_mvsy_host}, ports, HostType::Url);
    add_ip_ports(5, {bh_mvsy_host}, ports, HostType::Url);
  }
#endif
  return res;
}
